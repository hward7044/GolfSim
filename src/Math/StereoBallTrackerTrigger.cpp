...
StereoBallTrackerTrigger::StereoBallTrackerTrigger(
    ...
    cv::Rect searchRoiLeft,
    cv::Rect searchRoiRight,
    double minRadius,
    double maxRadius,
    double minCirc,
    int thresh,
    ...
    int armedWinSize,
    int graceMax,
    double impactVelThresh,
    double motionDispThresh
) : calib_(calib),
    state_(StereoTriggerState::SEARCHING),
    searchRoiLeft_(searchRoiLeft),
    searchRoiRight_(searchRoiRight),
    minBallRadiusPx_(minRadius),
    maxBallRadiusPx_(maxRadius),
    minCircularity_(minCirc),
    ballThreshold_(thresh),
    epipolarTolerancePx_(epipolarTol),
    disparityMinPx_(10.0),
    disparityMaxPx_(400.0),
    armedWindowSize_(armedWinSize),
    graceWindowMax_(graceMax),
    impactVelocityThreshold_(impactVelThresh),
    motionDisplacementThreshold_(motionDispThresh),
    lastKnown3DPos_(0.0, 0.0, 0.0),
    lastKnownLeft2D_(0.0, 0.0),
    lastKnownRight2D_(0.0, 0.0),
    lastKnownLeftRect2D_(0.0, 0.0),
    lastKnownRightRect2D_(0.0, 0.0),
    graceCounter_(0),
    confirmFrames_(0) {
...
cv::Point2d StereoBallTrackerTrigger::rectifyPoint(
    const cv::Point2d& pt,
...
bool StereoBallTrackerTrigger::triangulateCentroid(
    const cv::Point2d& leftPt,
    const cv::Point2d& rightPt,
    Eigen::Vector3d& out3D
) {
    cv::Point2d rectL = rectifyPoint(leftPt, calib_.K_L, calib_.D_L, calib_.R_L, calib_.P_L);
    cv::Point2d rectR = rectifyPoint(rightPt, calib_.K_R, calib_.D_R, calib_.R_R, calib_.P_R);
...
cv::Point2d StereoBallTrackerTrigger::project3DToLeft(const Eigen::Vector3d& pt3D) {
    cv::Mat ptHom = (cv::Mat_<double>(4, 1) << pt3D.x(), pt3D.y(), pt3D.z(), 1.0);
    cv::Mat proj = calib_.P_L * ptHom;
    double w = proj.at<double>(2, 0);
    if (std::abs(w) < 1e-6) return cv::Point2d(640, 400);
    return cv::Point2d(proj.at<double>(0, 0) / w, proj.at<double>(1, 0) / w);
}

cv::Point2d StereoBallTrackerTrigger::project3DToRight(const Eigen::Vector3d& pt3D) {
    cv::Mat ptHom = (cv::Mat_<double>(4, 1) << pt3D.x(), pt3D.y(), pt3D.z(), 1.0);
...
std::vector<BlobCandidate> StereoBallTrackerTrigger::extractCandidates(
    ...
    if (grayFrame.empty()) return candidates;

    cv::Rect safeRoi = searchROI & cv::Rect(0, 0, grayFrame.cols, grayFrame.rows);
    if (safeRoi.area() <= 0) return candidates;

    cv::Mat roiFrame = grayFrame(safeRoi);
    cv::Mat threshRoi;
    cv::threshold(roiFrame, threshRoi, ballThreshold_, 255, cv::THRESH_BINARY);
    ...
    for (const auto& contour : contours) {
        ...
        if (circularity >= minCircularity_) {
            ...
            if (m.m00 > 0.0) {
                ...
                cv::Rect globalBox(safeRoi.x + localBox.x, safeRoi.y + localBox.y, localBox.width, localBox.height);

                cv::Point2d rectCen = rectifyPoint(globalCentroid, K, D, R_rect, P_rect);
...
bool StereoBallTrackerTrigger::checkTrigger(const cv::Mat& leftFrame, const cv::Mat& rightFrame) {
    ...
    if (state_ == StereoTriggerState::SEARCHING) {
        auto leftCandidates = extractCandidates(grayL_, searchRoiLeft_, calib_.K_L, calib_.D_L, calib_.R_L, calib_.P_L);
        auto rightCandidates = extractCandidates(grayR_, searchRoiRight_, calib_.K_R, calib_.D_R, calib_.R_R, calib_.P_R);

        const BlobCandidate* bestL = nullptr;
        const BlobCandidate* bestR = nullptr;
        double minEpipolarErr = 1e9;
        ...
        for (const auto& candL : leftCandidates) {
            for (const auto& candR : rightCandidates) {
                ...
                if (epiErr <= epipolarTolerancePx_ && disparity >= disparityMinPx_ && disparity <= disparityMaxPx_) {
                    if (epiErr < minEpipolarErr) {
                        minEpipolarErr = epiErr;
                        bestL = &candL;
                        bestR = &candR;
                    }
                }
        ...
        if (bestL != nullptr && bestR != nullptr) {
            Eigen::Vector3d ball3D;
            if (triangulateCentroid(bestL->centroid, bestR->centroid, ball3D)) {
                lastKnown3DPos_ = ball3D;
                ...
                spdlog::info("[StereoBallTrackerTrigger] Ball locked! 3D centroid: ({:.3f}, {:.3f}, {:.3f}) m. State -> ARMED",
                             ball3D.x(), ball3D.y(), ball3D.z());
            }
        }
        ...
        latestDiag_ = {
            ...
            {"minEpipolarErr", (minEpipolarErr < 1e8) ? minEpipolarErr : 999.0},
            {"lastKnown3D", {lastKnown3DPos_.x(), lastKnown3DPos_.y(), lastKnown3DPos_.z()}}
        };
        return false;

    } else if (state_ == StereoTriggerState::ARMED) {
        int halfWin = armedWindowSize_ / 2;
        cv::Rect winL(static_cast<int>(lastKnownLeft2D_.x) - halfWin, static_cast<int>(lastKnownLeft2D_.y) - halfWin, armedWindowSize_, armedWindowSize_);
        cv::Rect winR(static_cast<int>(lastKnownRight2D_.x) - halfWin, static_cast<int>(lastKnownRight2D_.y) - halfWin, armedWindowSize_, armedWindowSize_);

        auto leftCandidates = extractCandidates(grayL_, winL, calib_.K_L, calib_.D_L, calib_.R_L, calib_.P_L);
        auto rightCandidates = extractCandidates(grayR_, winR, calib_.K_R, calib_.D_R, calib_.R_R, calib_.P_R);
        ...
        if (leftFound && rightFound) {
            ...
            if (bestL && bestR) {
                ...
                if (triangulateCentroid(bestL->centroid, bestR->centroid, curr3D)) {
                    ...
                    if (disp3D > motionDisplacementThreshold_) {
                        ...
                        spdlog::info("[StereoBallTrackerTrigger] Motion detected ({:.3f} m > threshold {:.3f} m)! State -> CONFIRMING",
                                     disp3D, motionDisplacementThreshold_);
                    } else {
            ...
            // Hold last known 3D position! Do not trigger, do not drop back to SEARCHING.
            spdlog::debug("[StereoBallTrackerTrigger] Single-camera occlusion immunity active (Left: {}, Right: {}). Holding ARMED state.",
            ...
            if (graceCounter_ > graceWindowMax_) {
                state_ = StereoTriggerState::SEARCHING;
                graceCounter_ = 0;
                spdlog::info("[StereoBallTrackerTrigger] Ball lost in both cameras for {} frames. Resetting -> SEARCHING", graceWindowMax_);
            }
        }

        latestDiag_ = {
            {"state", (state_ == StereoTriggerState::ARMED) ? "ARMED" : "CONFIRMING"},
            {"triggered", false},
            {"leftFound", leftFound},
        ...
        if (bothFound) {
            Eigen::Vector3d curr3D;
            if (triangulateCentroid(leftCandidates[0].centroid, rightCandidates[0].centroid, curr3D)) {
                confirmPositions3D_.push_back(curr3D);
                confirmFrames_++;

                // Estimate velocity over known inter-frame strobe interval dt = 0.001s (1 ms)
                double dt = 0.001; 
                ...
                if (speed >= impactVelocityThreshold_) {
                    ...
                    spdlog::info("[StereoBallTrackerTrigger] Impact velocity confirmed ({:.2f} m/s >= threshold {:.2f} m/s)! State -> CAPTURED",
                                 speed, impactVelocityThreshold_);
                    ...
                    spdlog::info("[StereoBallTrackerTrigger] Vibration/nudge rejected ({:.2f} m/s < threshold {:.2f} m/s). Returning -> ARMED",
                                 speed, impactVelocityThreshold_);
                    return false;
                }
            }
            ...
            if (!confirmPositions3D_.empty()) {
                state_ = StereoTriggerState::CAPTURED;
                spdlog::info("[StereoBallTrackerTrigger] Ball departed view at launch velocity! State -> CAPTURED");
